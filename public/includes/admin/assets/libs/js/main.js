...
$("#adminloginform").submit(function(e){
	e.preventDefault(); 
	//ajax_call("#adminloginform","admin/validateadmin","#admin-login-error","#admin-login-success","admin/dashboard","POST","json");
	var formData = new FormData($("#adminloginform")[0]);
	$.ajax({
		url: site_url+"admin/validateadmin", 
		type: 'POST',
		data: formData,
		processData: false,
		contentType: false,
		success: function(result){ 
			...
			if(res.category == "success"){
				var url = site_url+"admin/dashboard";
				window.location.replace(url);
			}else{
				var url = site_url+"admin/index";
				window.location.replace(url);
			}
		}
	});
